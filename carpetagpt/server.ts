...
const sanitizeFilename = (value: string, fallback = "omni_download") => {
  const safe = value
    .replace(/[<>:"/\\|?*\u0000-\u001F]/g, "")
    .replace(/\s+/g, "_")
    .trim();
...
const pipeFetchBody = (body: ReadableStream<Uint8Array> | null, res: express.Response) => {
  if (!body) {
    res.status(502).send("The remote server did not return a downloadable body");
    return;
  }
...
async function startServer() {
  ...
  app.post("/api/info", async (req, res) => {
    try {
      ...
      if (typeof url !== "string" || !isValidHttpUrl(url)) {
        return res.status(400).json({ error: "Ingresa una URL valida que comience con http:// o https://" });
      }
      ...
      if (!info || formats.length === 0) {
         return res.status(400).json({ error: "No se pudo extraer el enlace. Puede que la plataforma este bloqueando o que sea una red no soportada aun." });
      }
  ...
  app.get("/api/download", async (req, res) => {
    try {
      const { url, ext = "mp4", title, mp3 } = req.query;
      ...
      // Always proxy to control headers and avoid CORS/403
        ...
        if (!fetchRes.ok) {
          return res.status(fetchRes.status).send(`Remote server rejected the download (${fetchRes.status})`);
        }
        
        let targetExt = mp3 === 'true' ? 'mp3' : String(ext);
        let filename = `video_omni_download.${targetExt}`;
        if (title && typeof title === 'string') {
          let safeTitle = sanitizeFilename(title, "video");
        ...
        if (mp3 === 'true') {
            ...
            if (!fetchRes.body) {
              return res.status(502).send("The remote server did not return a downloadable body");
            }

            return ffmpeg(Readable.fromWeb(fetchRes.body as any))
              .inputFormat(ext === 'mp4' ? 'mp4' : String(ext)) // Handle simple case
              .toFormat('mp3')
              .on('error', (err) => { 
            ...
            if (remoteType) res.setHeader('Content-Type', remoteType);
            else if (ext === 'mp3') res.setHeader('Content-Type', 'audio/mpeg');
            else if (['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(String(ext))) res.setHeader('Content-Type', `image/${ext === 'jpg' ? 'jpeg' : ext}`);
            else res.setHeader('Content-Type', 'video/mp4');
            return pipeFetchBody(fetchRes.body, res);
        }
    } catch (error: any) {
      console.error(error);
  ...
  app.get("/api/proxy-image", async (req, res) => {
    try {
      ...
      const fetchRes = await fetch(url, { headers: fetchHeaders });
      if (!fetchRes.ok) {
        return res.status(fetchRes.status).send("Remote image request failed");
      }
