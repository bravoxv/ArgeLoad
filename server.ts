...
async function startServer() {
  ...
  app.post("/api/info", async (req, res) => {
    ...
    try {
      ...
      if (domain.includes('tiktok.com')) {
        try {
          ...
          if (tt?.video) {
            return res.json({
              title: tt.title || "Video de TikTok",
              author: tt.author || "Usuario de TikTok",
              thumbnail: tt.cover || "",
              platform: 'tiktok',
              formats: [
                { itag: 137, mimeType: 'video/mp4', qualityLabel: 'HD Sin Marca de Agua', hasVideo: true, hasAudio: true, container: 'mp4', url: tt.video },
    ...
    if (domain.includes('twitter.com') || domain.includes('x.com')) {
      ...
      if (tw?.url?.length > 0) {
        return res.json({
          title: tw.title || "Video de Twitter/X", thumbnail: tw.thumbnail || "", platform: 'twitter',
    ...
    if (domain.includes('threads.net')) {
      ...
      if (thr?.result?.length > 0) {
        return res.json({
          title: "Video/Imagen de Threads", thumbnail: thr.result[0].url, platform: 'threads',
